...
export default function PostCard({ post }: { post: Post }) {
  ...
  const followMutation = useMutation({
    mutationFn: () => apiService.social.followCompany({ companyId: post.company?.id ?? '' }),
    onSuccess: () => {
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="h-9 w-9 rounded-full bg-muted flex items-center justify-center shrink-0 overflow-hidden">
            {post.company?.logo
              ? <Image src={post.company.logo} alt={post.company.name} className="h-full w-full object-cover" />
